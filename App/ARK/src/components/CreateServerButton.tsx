...
type ServerType = 'paper' | 'spigot' | 'vanilla' | 'fabric' | 'forge' | 'purpur' | 'velocity' | 'waterfall' | 'bungeecord' | 'manual' | string;
...
export default function CreateServerButton() {
  ...
  const stepLabels = ['Map', 'Session & Port', 'Settings'];
    ...
    const loadSettings = async () => {
      if (window.electronAPI) {
        try {
          ...
          setServerPort(appSettings.defaultPort ?? 7777);
  ...
  const loadVersions = async () => {
    ...
    try {
      ...
      const versionsList = window.electronAPI.server.getArkMaps
        ? await window.electronAPI.server.getArkMaps()
        : [];
      setVersions(versionsList.length > 0 ? versionsList : ['TheIsland', 'Ragnarok', 'TheCenter', 'Valguero', 'CrystalIsles', 'LostIsland', 'Fjordur']);
      const map = versionsList[0] || 'TheIsland';
      setServerType(map as ServerType);
      if (!selectedVersion) setSelectedVersion(map);
    } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const result = await createServer(
        ...
        serverPort,
        null,
        (sessionName && sessionName.trim()) ? sessionName.trim() : name
      );
      if (result.success) {
        if (autoStart) {
        ...
        setShowInput(false);
        setServerName("");
        setServerType('TheIsland');
        setSelectedVersion('TheIsland');
        setSessionName("");
        setManualJarPath(null);
        setRamGB(settings?.defaultRAM || 4);
        setServerPort(settings?.defaultPort ?? 7777);
        setCreateStep(0);
        setErrorMessage(null);
        setWarningMessage(null);
        setPortMessage(null);
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-8" style={{ pointerEvents: 'auto' }}>
        ...
        <div className="space-y-6 relative z-10">
          ...
          {createStep === 0 && showImportFlow && (
            <div>
              ...
              <p className="text-xs text-text-muted font-mono mb-4">
                Choose a folder that contains an existing ARK server. It will be copied into Nodexity ARK.
              </p>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs text-text-muted font-mono mb-1">Server name (in Nodexity ARK)</label>
                  <input
                    type="text"
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div className="text-text-secondary font-mono text-sm">
                  ...
                  />
                  <p className="text-xs text-text-muted mt-1">Recommended: 7777 (1024-65535)</p>
                </div>
          ...
          {createStep === 0 && !showImportFlow && !showPresetFlow && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-text-secondary font-mono mb-2">Server Name</label>
                <input
                  type="text"
                  value={serverName}
                  onChange={(e) => setServerName(e.target.value)}
              ...
              <div>
                <label className="block text-sm text-text-secondary font-mono mb-2">Map</label>
                {loadingVersions && versions.length === 0 ? (
                  <div className="text-text-muted font-mono text-sm py-4">Loading maps...</div>
                ) : (
                <select
                  value={serverType}
                  onChange={(e) => { setServerType(e.target.value); setSelectedVersion(e.target.value); }}
                  className="w-full bg-background-secondary border border-border px-4 py-3 text-text-primary font-mono text-sm focus:outline-none focus:border-accent/50 rounded select-custom"
                  disabled={isCreating}
                >
                  {versions.map((m) => (
          ...
          {/* Step 2: Session & Port or Manual JAR */}
          ...
          {createStep === 1 && serverType !== 'manual' ? (
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-text-secondary font-mono mb-2">Session Name</label>
                <input
                  type="text"
                  value={sessionName || serverName}
                  onChange={(e) => setSessionName(e.target.value)}
                  placeholder="Display name in server list"
                  className="w-full bg-background-secondary border border-border px-4 py-3 text-text-primary font-mono text-sm focus:outline-none focus:border-accent/50 rounded"
                  disabled={isCreating}
                />
              </div>
              <div>
                <label className="block text-sm text-text-secondary font-mono mb-2">Port</label>
                <input
                  type="number"
                  min="1024"
                  max="65535"
                  ...
                  onChange={(e) => {
                    setPortAutoApplied(false);
                    setPortMessage(null);
                    setServerPort(parseInt(e.target.value) || 7777);
                  }}
                  disabled={isCreating}
                  className="w-full bg-background-secondary border border-border px-4 py-3 text-text-primary font-mono text-sm focus:outline-none focus:border-accent/50 rounded"
                />
                <p className="text-xs text-text-muted mt-1">Game port (default 7777)</p>
              </div>
              <p className="text-xs text-text-muted font-mono">Map: <span className="text-text-primary">{serverType}</span></p>
            </div>
          ) : null}
