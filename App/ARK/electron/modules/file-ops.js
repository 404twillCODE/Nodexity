...
// Optional: only needed for Minecraft NBT; ARK app does not use it
let nbt = null;
function getNbt() {
  if (nbt === null) {
    try {
...
async function readServerFileNbt(serverName, filePath) {
  const nbtLib = getNbt();
  if (!nbtLib) {
    return { success: false, error: 'NBT support not available (prismarine-nbt not installed)' };
  }
  try {
    const settings = await config.getAppSettings();
    const serversDir = settings.serversDirectory || config.SERVERS_DIR;
    ...
    const { parsed, type } = await nbtLib.parse(rawBuffer);
...
async function writeServerFileNbt(serverName, filePath, parsed, nbtFormat) {
  ...
  try {
    ...
    const rawBuffer = await fs.readFile(fullPath);
    const original = await nbtLib.parse(rawBuffer);

    // Write back using the original format (big/little/bedrockLevel)
    const outBuffer = fsSync.createWriteStream ? nbtLib.writeUncompressed(original.parsed, original.type) : nbtLib.writeUncompressed(original.parsed);
...
async function installModrinthPlugin(serverName, projectId, minecraftVersion) {
  return { success: false, error: 'Plugins are not used on ARK servers' };
  try {
...
async function installModrinthMod(serverName, projectId, minecraftVersion) {
  return { success: false, error: 'Use Steam Workshop for ARK mods' };
  try {
    const supportsMods = await checkJarSupportsMods(serverName);
    if (!supportsMods.supportsMods) {
...
async function installGeyserFloodgate(serverName, serverPort = 25565) {
  return { success: false, error: 'Not applicable to ARK servers' };
  try {
    const supportsPlugins = await checkJarSupportsPlugins(serverName);
    if (!supportsPlugins) {
...
// ARK: read from Saved/Config/WindowsServer/GameUserSettings.ini (INI format)
...
function stringifyIni(properties) {
  const sectionKeys = { 'ServerSettings': ['SessionName', 'Port', 'QueryPort', 'MaxPlayers', 'ServerAdminPassword', 'ServerPassword', 'ServerHardcore', 'ServerCrosshair', 'ServerForceNoHUD', 'ServerThirdPersonPlayer', 'ServerAllowFlyerCarryPvE', 'ProximityChat', 'ShowMapPlayerLocation', 'AllowRaidDinoFeeding', 'PreventDownloadDinos', 'PreventDownloadItems', 'PreventDownloadSurvivors', 'PreventUploadDinos', 'PreventUploadItems', 'PreventUploadSurvivors', 'PreventSpawnAnimations', 'ForceAllowCaveFlyers', 'DisableStructureDecayPvE', 'AllowUnlimitedRespecs', 'AllowIntegratedSaddleBuff', 'AllowFlyingStaminaRecovery', 'AllowRaidDinoFeeding', 'ActiveMods', 'MapPlayerLocation'] };
  const seen = new Set();
  let out = '';
  ...
  return out || '[ServerSettings]\n';
}

async function getServerProperties(serverName) {
  try {
    const result = await readServerFile(serverName, 'Saved/Config/WindowsServer/GameUserSettings.ini');
    if (!result.success) {
      return { success: false, error: result.error };
    }
    const properties = parseIni(result.content);
...
async function updateServerProperties(serverName, properties) {
  try {
    ...
    if (result.success) {
      const existing = parseIni(result.content);
      Object.assign(existing, properties);
      content = stringifyIni(existing);
    } else {
      content = stringifyIni(properties);
    }
    return await writeServerFile(serverName, 'Saved/Config/WindowsServer/GameUserSettings.ini', content);
  } catch (error) {
    return { success: false, error: error.message };
  }
}
